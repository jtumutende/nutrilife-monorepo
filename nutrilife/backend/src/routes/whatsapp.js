...
router.post('/webhook', async (req, res) => {
  ...
  const user = await prisma.user.findFirst({ where: { phone: { contains: phone.slice(-9) } } }).catch(() => null);
  ...
  try {
    if (text.match(/^(hi|hello|hey|start)/)) {
      ...
      const meals = await prisma.meal.findMany({ where: { isAvailable: true }, take: 6, orderBy: { isPopular: 'desc' } });
      const list  = meals.map((m, i) => `${i + 1}. ${m.emoji} *${m.name}* — UGX ${m.price.toLocaleString()} (${m.calories} kcal)`).join('\n\n');
      await sendMessage(phone, `🥗 *Today's Menu:*\n\n${list}\n\nVisit: ${process.env.FRONTEND_URL}`);
    } else if (text.includes('track')) {
      if (!user) {
        await sendMessage(phone, `Please create an account first:\n${process.env.FRONTEND_URL}/pages/register.html`);
      } else {
        const orders = await prisma.order.findMany({ where: { userId: user.id }, orderBy: { createdAt: 'desc' }, take: 3 });
        if (!orders.length) {
          await sendMessage(phone, `📦 No orders found. Type *MENU* to browse meals!`);
        } else {
          const info = orders.map(o => `• *${o.orderNumber}*: ${o.status.replace('_', ' ')} — UGX ${o.totalAmount.toLocaleString()}`).join('\n');
      ...
      await sendMessage(phone, `🤔 I did not understand that, ${name}.\n\nTry: *MENU* · *ORDER* · *TRACK* · *HELP*`);
    }
  } catch (err) { console.error('WhatsApp bot error:', err.message); }
  res.sendStatus(200);
});
