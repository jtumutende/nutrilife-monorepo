...
async function main() {
  ...
  await prisma.user.upsert({
    where: { email: 'admin@nutrilife.ug' }, update: {},
    create: { firstName: 'NutriLife', lastName: 'Admin', email: 'admin@nutrilife.ug', password: adminPass, phone: '256700000001', role: 'ADMIN', plan: 'Elite', isActive: true }
  });

  const userPass = await bcrypt.hash('password123', 12);
  await prisma.user.upsert({
    where: { email: 'john@nutrilife.ug' }, update: {},
    create: { firstName: 'John', lastName: 'Doe', email: 'john@nutrilife.ug', password: userPass, phone: '256700123456', plan: 'Pro', goal: 'Weight Loss', age: 28, weight: 72, height: 175, gender: 'Male', address: 'Nakasero, Kampala', isActive: true }
  });
  ...
  for (const meal of meals) {
    await prisma.meal.upsert({
      where: { name: meal.name }, update: {},
      create: { ...meal, isAvailable: meal.isAvailable ?? true, isPopular: meal.isPopular ?? false }
    });
  }
