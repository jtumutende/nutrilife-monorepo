...
async function sendMessage(toPhone, text) {
  ...
  if (!client) {
    console.log('📱 WhatsApp skipped — Twilio not configured yet');
    return;
  }
...
async function sendPaymentConfirmation(phone, firstName, amount, orderNum) {
  await sendMessage(phone,
    `💳 *Payment Received!*\n\n` +
    `Hi ${firstName}, we received *UGX ${Number(amount).toLocaleString()}* ` +
    `for order *${orderNum}*.\n\n` +
